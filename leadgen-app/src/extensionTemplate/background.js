...
//
// These defaults are baked in for YOUR account specifically - they were
// substituted in when you downloaded this extension from Xeven Leads, so
// it works immediately with zero setup. They're written to
// chrome.storage.sync on install, and getSettings() also falls back to
// them directly as a safety net.
...
async function createTrackedEmail({ subject, recipients, sender, provider }) {
  ...
  const res = await fetch(`${backendUrl}/api/tracker/emails`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-api-key": apiKey,
    },
    body: JSON.stringify({ subject, recipients, sender, provider: provider || "hostinger" }),
  });
...
async function fetchStats() {
  ...
  const res = await fetch(`${backendUrl}/api/tracker/stats`, {
